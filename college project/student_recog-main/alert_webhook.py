...
def build_parent_contacts(payload: dict) -> list[dict]:
    ...
    if isinstance(payload.get("parent_contacts"), list):
        for contact in payload["parent_contacts"]:
...
def build_teacher_contacts(payload: dict) -> list[dict]:
    contacts = []
    if isinstance(payload.get("teacher_contacts"), list):
        for contact in payload["teacher_contacts"]:
            if not isinstance(contact, dict):
                continue
            contacts.append(
                {
